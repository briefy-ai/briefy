...
class GlobalExceptionHandler {
    ...
    fun handleShareLinkNotFound(ex: ShareLinkNotFoundException): ResponseEntity<ErrorResponse> {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
            .body(ErrorResponse(
                status = HttpStatus.NOT_FOUND.value(),
                error = "Not Found",
                message = ex.message ?: "Share link not found"
            ))
    }

    @ExceptionHandler(ShareLinkExpiredException::class)
    fun handleShareLinkExpired(ex: ShareLinkExpiredException): ResponseEntity<ErrorResponse> {
