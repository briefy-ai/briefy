...
- **Local narration testing**: `docker compose up minio` then `cd apps/api && ./gradlew bootRun`. Full docker-compose won't work for audio playback because presigned URLs resolve to `http://minio:9000` which the host browser can't reach.
